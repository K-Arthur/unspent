...
serve(async (req) => {
  ...
  try {
    ...
    if (!priceId || (configuredPriceIds.length > 0 && !configuredPriceIds.includes(priceId))) {
      return new Response(
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: successUrl || `${webUrl}/upgrade?success=true`,
      cancel_url: cancelUrl || `${webUrl}/upgrade?canceled=true`,
