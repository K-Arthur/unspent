      ...
      <div className="max-w-6xl mx-auto">
            ...
            <ul className="space-y-2">
              {links.product.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-text-secondary hover:text-text link-focus transition-colors"
                    {...(link.href.startsWith('http') ? { rel: 'noopener' } : {})}
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <nav aria-label="Company">
            ...
            <ul className="space-y-2">
              {links.company.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-text-secondary hover:text-text link-focus transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
        ...
        <div className="border-t border-accent pt-8 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-sm text-text-secondary">
            &copy; {currentYear} Unspent. All rights reserved.
          </p>
          <div className="flex items-center gap-4">
            <a
              href="https://twitter.com"
              className="text-text-secondary hover:text-text link-focus transition-colors w-11 h-11 flex items-center justify-center"
              aria-label="Twitter"
